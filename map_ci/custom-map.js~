...
var availableStores = [];
var availableClass = [];
var base_url = 'http://bitgazetteer.com/bitgazetteerstest/';
var no_logo_atm;
var no_logo_store;
var no_logo_class;
var no_logo_offer;
...
$(document).ready(function() {
    $.ajax({
        ...
        success: function(return_data) {
            $.map(return_data.data.cointype, function(value, index) {
                coin_arr.push(value);
            });
        }
    });
...
function createMarker(marker, name, address1, address2, contact, latlng, coin, logo, read, wrk_hrs, c, sc,web_link,class_spe) {
    ...
    if (coin == null)
        ...
        var dis = '<p> <b>Accepted Coins -</b>' + coin_arr[coin] + '</p>';
  ...
  var star = '';
    ...
    google.maps.event.addListener(marker, 'click', function() {
        var iwContent = open_div + logo_dis + name_lable + dis + contact_dis + contact+ web_link +working_hrs +class_data_dis+ footer ;
...
function create_marker_by_search(search_results) {
    ...
    $.map(search_results, function(value, index) {
        i++;
        var coin = value.Atm.accepting_coins;
        var id = value.Atm.id;
        ...
        var category = value.Atm.monday;
        var subcategory = value.Atm.monday;
        var wrk_hrs = {
            friday: friday,
            monday: monday,
            saturday: saturday,
...
function create_marker_offers(search_results) {
    ...
    $.map(search_results.data.offers, function(value, index) {
        ...
        var contact = value.Store.contact_no;
        var coin = value.Store.accepting_coins;
        var icon = "/assets/img/offers.png";
        var latlng = new google.maps.LatLng(lat, lon);    
        var marker = new google.maps.Marker({
            map: map,
            zoom: 10,
